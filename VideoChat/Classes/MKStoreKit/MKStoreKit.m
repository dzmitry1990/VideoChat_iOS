...
- (BOOL)isSubscriptionForProductActive:(NSString *)productId {
    NSDate *now = [NSDate date];
    
    BOOL isActive = [self isProductPurchased:productId] && [[self expiryDateForProduct:productId] isLaterThanDate:now];
...
- (void)initiatePaymentRequestForProductWithIdentifier:(NSString *)productId {
  ...
  if (![SKPaymentQueue canMakePayments]) {
    ...
    UIAlertController *controller = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"In App Purchasing Disabled", @"")
     ...
     [((UIApplication *)[UIApplication performSelector:@selector(sharedApplication)]).keyWindow.rootViewController
...
@implementation MKStoreKit (AnySubscriptionActive)

- (BOOL)isSubscriptionForAnyProductActive {
    BOOL active = NO;
    
    NSDate *now = [NSDate date];
    for (SKProduct *product in self.availableProducts) {
        active = [self isProductPurchased:product.productIdentifier] && [[self expiryDateForProduct:product.productIdentifier] isLaterThanDate:now];
        if (active) {
            break;
        }
    }
